<#
.SYNOPSIS
    Reports and optionally saves the last 100 application events that are categorized as a warning or critical.
...
.NOTES
    Name: Get-EventsAppWarningCritical
    Author: Visusys
    Version: 1.0.0
    DateCreated: 2021-11-07T05:34:21.000-05:00

.EXAMPLE
    Get-EventsAppWarningCritical -SaveToDisk

.LINK
    https://github.com/visusys
#>
function Get-EventsAppWarningCritical {
    [CmdletBinding()]
    param (
    ...
    $events = Get-WinEvent -ComputerName $ComputerName –FilterHashtable @{logname='application'; level=1,2,3} –MaxEvents 200
    $formatting = Format-Table -AutoSize -Wrap -InputObject $events
    $filename = 'EventLogReport-Application.txt'
